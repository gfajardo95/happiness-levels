...
public class DatastoreWriter {
    static class CreateEntityFn extends DoFn<String, Entity> {
        ...
        private Entity makeEntity(String content) {
            ...
            com.google.datastore.v1.Key key = com.google.datastore.v1.Key.newBuilder()
                    .addPath(com.google.datastore.v1.Key.PathElement.newBuilder().setKind("world_city")
    ...
    /*
     * Run command: mvn compile exec:java -D exec.mainClass=DatastoreWriter -D exec.
     * args="--runner=DataFlowRunner --project=happiness-level --gcpTempLocation=gs://dataflow-happiness-level/writer/tmp --input=gs://world_cities/worldcities.csv --workerMachineType=n1-standard-1 --maxNumWorkers=2"
     * -Pdataflow-runner
     *
     * Writes a .csv file in a Cloud Storage bucket to Cloud Datastore. The .csv
     * file has columns: city_ascii, country, iso2, iso3
     */
    public static void main(String... args) {
        Options options = PipelineOptionsFactory.fromArgs(args).withValidation().as(Options.class);
        Pipeline p = Pipeline.create(options);

        p.apply(TextIO.read().from(options.getInput())).apply(ParDo.of(new CreateEntityFn()))
